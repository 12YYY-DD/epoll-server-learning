...
Reactor::Reactor(int port): pool(4){// 构造函数，初始化服务器4线程
    ...
    int opt = 1;
    setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));// 设置地址复用，避免重启服务器时绑定端口失败
...
void Reactor:: run(){
    ...
    while (true) {
        // 3. 等待事件
        int n = epoll_wait(epfd, events, 1024, 1000);// 等待事件，超时时间 1000ms
        checkTimeouts();// 定期检查连接的超时情况，关闭超时的连接
...
void Reactor::handleWrite(int fd){// 处理可写事件，发送数据给客户端
    ...
    while (!conn->writebuffer.empty()) {
        ...
        conn->lastActive = chrono::steady_clock::now();// 更新上次活跃时间
    } 
    //写完了，取消监听
...
void Reactor:: checkTimeouts(){
    ...
    for (auto it = conns.begin(); it != conns.end(); ) {
        auto conn = it->second;
        auto duration = chrono::duration_cast<chrono::seconds>(now - conn->lastActive).count();
        if (duration > 10) { // 超过 10 秒没有活动了，认为超时了
            log("INFO", "timeout fd=" + std::to_string(conn->fd));
            int fd = it->first;
            ++it;// 先把迭代器移到下一个位置
            closeConnection(fd);// 关闭超时的连接 
        } 
        else {
            ++it;
        }
